...
Wow! I typed some words. : )



Class Notes from Wed 9/23
...
KidsFeetBoys <- KidsFeet %>%
  filter(sex == "B")

KidsFeetCrazy <- KidsFeet %>%
  filter(sex == "G", length > 24.50, domhand == "L")


KidsFeetBoys <- KidsFeet %>%
  filter(sex == "B") %>%
  select(name, birthmonth, birthyear, domhand, width, length)
...
KidsFeet %>%
  group_by(sex) %>%
  summarise(Min = min(width), Max = max(width), `Sample Size` = n()) %>%
  pander()


Kids3 <- KidsFeet %>%
  mutate(season = case_when(birthmonth %in% c(12,1,2) ~ "Winter",
...
Use the [Independent Sample t Test](tTests.html#independent-samples-t-test) to test for the equality of means in "Y" for each group "A" and "B."

See my [High School Seniors](./Analyses/t Tests/HighSchoolSeniors.html)


Use the [Wilcoxon Rank Sum Test](WilcoxonTests.html) to test for equality of medians, or "stochastical dominance." 

See my [Recalling Words](./Analyse/Wilcoxon Tests/RecallingWords.html)
...
Put a practice test <span style="font-size:18pt;background-color:yellow;color:blue;">question or two here.</span>
